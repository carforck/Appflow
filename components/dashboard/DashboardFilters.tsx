...
export function DashboardFilters({ filters, projectOptions, empresas, onChange, onReset }: Props) {
  const hasActive = Object.values(filters).some(Boolean);

  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">

      {/* Empresa — filtro maestro, primero en el orden */}
      {empresas.length > 0 && (
        <div className="flex flex-col gap-1 w-full sm:w-auto sm:min-w-[160px]">
          <label className={LABEL}>Empresa</label>
          <select
            value={filters.empresa}
            onChange={(e) => onChange({ empresa: e.target.value, project_id: '' })}
            className={SELECT}
          >
